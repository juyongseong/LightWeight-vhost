...
extern void print_vhost_net(const struct vhost_net *net, const char * str);
...
extern void print_vhost_net(const struct vhost_net *net, const char * str)
{
        ...
        struct socket * sock2;
        printk(KERN_ALERT "%s \n", str);
        printk(KERN_ALERT "net: %p, pdev: %p, nvq: tx-> %p, rx-> %p, vq: tx-> %p, rx-> %p ", net, pdev, nvq, nvq2, vq, vq2);
...
static void handle_tx_copy(struct vhost_net *net, struct socket *sock)
{
	...
	bool sock_can_batch = (sock->sk->sk_sndbuf == INT_MAX);
	 /*
	 print_vhost_net(net, __func__);
	 */                                

	for (;;) {
		bool busyloop_intr = false;

		/*printk(KERN_ALERT "handle_tx_copy while");
		*/
...
static void handle_tx_zerocopy(struct vhost_net *net, struct socket *sock)
{
	...
	*/
	for (;;) {
		bool busyloop_intr;
		/*
		printk(KERN_ALERT "handle_tx_zerocopy while");
		*/
...
static void handle_rx(struct vhost_net *net)
{
	...
	*/                  
	...
	while ((sock_len = vhost_net_rx_peek_head_len(net, sock->sk,
		...
		/*
		printk(KERN_ALERT "handle_rx while");
		*/
		sock_len += sock_hlen;
...
static ssize_t do_kboard_enqueue(struct file *write, const char __user* usr, size_t len, loff_t *data){
        ...
        int clip;
        int res;
	printk(KERN_INFO "hellooooo do_kboard_enqueue");
	mytest=0;
        writerCall++;
...
static int kboard_read(void){
        int res;
        int i;
	printk(KERN_INFO "hellooo kboard_read");
	mytest=0;
...
static ssize_t do_kboard_read(struct file *read, char __user *usr, size_t size, loff_t *data){
        ...
        unsigned long int bufSize;
        int res;
        printk(KERN_INFO "helloooo do_kboard_read");
	mytest=0;
        readerCall++;
