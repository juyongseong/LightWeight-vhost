...
static inline void fperf_start(const char * str) {
        int i;
...
unsigned int vhost_chr_poll(struct file *file, struct vhost_dev *dev,poll_table *wait)
{
	fperf_start(__func__);
	unsigned int mask = 0;
...
static int vq_access_ok(struct vhost_virtqueue *vq, unsigned int num, struct vring_desc __user *desc,struct vring_avail __user *avail,struct vring_used __user *used)

{
	fperf_start(__func__);
	size_t s = vhost_has_feature(vq, VIRTIO_RING_F_EVENT_IDX) ? 2 : 0;
	int ret = access_ok(VERIFY_READ, desc, num * sizeof *desc) &&
...
static int iotlb_access_ok(struct vhost_virtqueue *vq,int access, u64 addr, u64 len, int type)
{
	fperf_start(__func__);
...
int vq_iotlb_prefetch(struct vhost_virtqueue *vq)
{
	...
	int ret =iotlb_access_ok(vq, VHOST_ACCESS_RO, (u64)(uintptr_t)vq->desc,
...
static int vq_log_access_ok(struct vhost_virtqueue *vq,void __user *log_base)
{
	fperf_start(__func__);
	size_t s = vhost_has_feature(vq, VIRTIO_RING_F_EVENT_IDX) ? 2 : 0;
	int ret = vq_memory_access_ok(log_base, vq->umem,
